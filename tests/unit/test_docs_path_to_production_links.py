"""Link-checker for ``docs/path-to-production.md`` (Plan 4 T44).
...
# No forward references expected for path-to-production.md — every linked path
...
def _repo_root() -> Path:
    # tests/unit/test_docs_path_to_production_links.py  →  tests/unit  →  tests  →  repo root
    return Path(__file__).resolve().parents[2]


def _doc_path() -> Path:
    return _repo_root() / "docs" / "path-to-production.md"
...
def test_docs_path_to_production_md_is_non_trivially_long() -> None:
    doc = _doc_path()
    assert doc.exists(), f"{doc} does not exist"
    line_count = sum(1 for _ in doc.read_text(encoding="utf-8").splitlines())
    assert line_count > 50, f"docs/path-to-production.md too short: {line_count} lines"


def test_docs_path_to_production_md_links_resolve() -> None:
    doc = _doc_path()
    ...
    assert not missing, (
        "docs/path-to-production.md has unresolved link targets:\n"
