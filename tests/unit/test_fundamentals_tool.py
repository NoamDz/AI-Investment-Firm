...
script in fixture mode (FIRM_FUNDAMENTALS_FIXTURE=1) to write
tests/fixtures/fundamentals_fixture.parquet at test-collection time.
"""
...
FIXTURE_PARQUET = (
    Path(__file__).parent.parent / "fixtures" / "fundamentals_fixture.parquet"
)
SCRIPT = (
...
def fixture_parquet(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """Run precompute script in fixture mode; return path to the parquet file."""
    env = {**os.environ, "FIRM_FUNDAMENTALS_FIXTURE": "1"}
    ...
    assert FIXTURE_PARQUET.exists(), (
        f"Expected parquet at {FIXTURE_PARQUET} but it was not created."
    )
    return FIXTURE_PARQUET


@pytest.fixture(scope="session")
def tool(fixture_parquet: Path) -> "FundamentalsTool":  # noqa: F821
...
def test_get_ratio_returns_decimal_for_known_pair(tool: "FundamentalsTool") -> None:  # noqa: F821
...
def test_get_ratio_raises_for_unknown_ticker(tool: "FundamentalsTool") -> None:  # noqa: F821
...
def test_get_ratio_raises_for_unknown_ratio_name(tool: "FundamentalsTool") -> None:  # noqa: F821
...
def test_get_ratio_uses_as_of_to_select_latest_filing(tool: "FundamentalsTool") -> None:  # noqa: F821
...
def test_get_ratio_raises_when_no_entry_on_or_before_as_of(
    tool: "FundamentalsTool",  # noqa: F821
) -> None:
