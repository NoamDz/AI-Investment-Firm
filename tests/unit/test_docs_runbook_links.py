"""Link-checker for ``docs/runbook.md`` (Plan 4 T46).
...
# No forward references expected for runbook.md — every linked path should
# exist on disk at the time the doc is written.
...
def _repo_root() -> Path:
    # tests/unit/test_docs_runbook_links.py  →  tests/unit  →  tests  →  repo root
    return Path(__file__).resolve().parents[2]


def _doc_path() -> Path:
    return _repo_root() / "docs" / "runbook.md"
...
def test_docs_runbook_md_is_non_trivially_long() -> None:
    doc = _doc_path()
...
def test_docs_runbook_md_links_resolve() -> None:
    doc = _doc_path()
    ...
    assert not missing, (
        "docs/runbook.md has unresolved link targets:\n"
